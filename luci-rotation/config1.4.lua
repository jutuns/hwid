...
function otw(world,id)
    ...
    if id ~= "" and not cantJoinDoor then
        ...
        if getTile(bot.x,bot.y).fg == 6 and not nuked then
            ...
            sleep(2000)
            if getTile(bot.x,bot.y).fg == 6 and not cantJoinDoor then
                nukeWorldInfo(webhookNuked,bot.name:upper() .. " world "..world.." cannot join world. @everyone")
                sleep(100)
                cantJoinDoor = true
            end
        end
...
function reconnect(world,id,x,y)
    if bot.status ~= BotStatus.online or bot:getPing() == 0 then
        botInfo(webhookOffline,":red_circle: NO."..indexBot.." "..bot.name.." bot status is "..bot.status.." @everyone")
    ...
    if differentWorldPNB then
        ...
        if not nuked then
            ...
            if findItem(itmId) >= tileNumber and bot:getWorld().name == worldBreak:upper() then
                ...
                while bot:getInventory():findItem(itmId) > tileNumber and bot:getInventory():findItem(itmSeed) <= 190 and bot.gem_count <= minimumGem and bot.x == ex and bot.y == ye do
        ...
        if findItem(itmId) >= tileNumber and bot:getWorld().name == world:upper() then
            ...
            while findItem(itmId) > tileNumber and findItem(itmSeed) <= 190 and bot.gem_count <= minimumGem and bot.x == ex and bot.y == ye do
                if bot.level >= removeBotAfterLevel then
                    botEvents("Bot removed reach level "..bot.level)
                    removeBot(bot.name)
                    bot:stopScript()
...
function plant(world)
    ...
    for _,tile in pairs(bot:getWorld():getTiles()) do
        ...
        if getTile(tile.x,tile.y).fg == 0 and isPlantable(getTile(tile.x,tile.y)) and findItem(itmSeed) > 0  and bot:getWorld():hasAccess(tile.x,tile.y) and bot:getWorld().name == world:upper() then
            findPath(tile.x,tile.y)
            if getTile(tile.x,tile.y).fg == 0 and bot.x == tile.x and bot.y == tile.y then
                place(itmSeed,0,0)
                sleep(delayPlant)
...
function harvest(world)
    ...
    elseif autoFill then
        for _,tile in pairs(bot:getWorld():getTiles()) do
            ...
            if (tile:canHarvest() or (tile.fg == 0 and tile.y ~= 0 and isPlantable(getTile(tile.x,tile.y)))) and bot:isInWorld(world:upper()) and bot:getWorld():hasAccess(tile.x,tile.y) > 0 then
                if bot.level >= removeBotAfterLevel then
                    removeBot(bot.name)
                    bot:stopScript()
                end
                ...
                if bot:getInventory():findItem(itmSeed) > 0 and getTile(tile.x,tile.y).fg == 0 and bot.x == tile.x and bot.y == tile.y and bot:getWorld().name == world:upper() then
                    place(itmSeed,0,0)
                    sleep(delayPlant)
                    reconnect(world,doorFarm,tile.x,tile.y - 1)
                end
