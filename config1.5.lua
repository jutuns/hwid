...
if activateScript then
    ...
    dividerWPickaxe = math.ceil(indexLast / #worldPickaxeList)
    worldPickaxe = worldPickaxeList[math.ceil(indexBot / dividerWPickaxe)]
...
function infoJutun()
    ...
    $fieldArray = @(
        @{
            ...
            value = "<:arrow:1160743652088365096> Username : ]]..response.."\n"..[[<:arrow:1160743652088365096> Total Bot : ]]..indexLast..[["
            inline = "false"
        }
    )
    $embedObject = @{
        title = "**ROTASI OLYMPUS V1.5**"
        color = "16777215"
        footer = $footerObject
...
function skipTutorial()
    if findItem(9640) > 0 and getBot().world:find(getBot().name:upper()) then
        place(9640,0,-1)
        sleep(5000)
    end
end
...
function warp(world,id)
    ...
    while getBot().world ~= world:upper() and not nuked do
        ...
        if cok == 10 then
            ...
            sleep(100)
            botInfo(webhookOffline,getBot().name.." ("..indexBot..")".." bot disconnect 2 minutes while server growtopia sucks @everyone")
            sleep(100)
            setJob("Resting")
            disconnect()
            sleep(5 * 60 * 1000)
    ...
    if nuked then
        nukeWorldInfo(webhookNuked,getBot().name:upper() .. " world "..world.." is nuked. @everyone")
    end
    cok = 0
...
function reconnect(world,id,x,y)
    ...
    if getBot().status ~= "online" then
        botInfo(webhookOffline,":red_circle: "..getBot().name.." ("..indexBot..")".." bot status is "..getBot().status.." @everyone")
        sleep(100)
        while getBot().status ~= "online" do
            connect()
            sleep(10000)
...
function pnb(world)
    if differentWorldPNB then
        ...
        sleep(100)
        if not nuked and getBot().world:upper() == worldBreak:upper() then
            if randomChat then
            ...
            if findItem(itmId) >= tileNumber and getBot().world:upper() == worldBreak:upper() then
                ...
                while findItem(itmId) > tileNumber and findItem(itmSeed) <= 190 and math.floor(getBot().x / 32) == ex and math.floor(getBot().y / 32) == ye do
                    ...
                    if getBot().level >= removeBotAfterLevel then
                        botEvents("Bot removed reach level "..getBot().level)
                        removeBot(getBot().name)
                        error()
                    end
                    while tilePlace(ex,ye) do
                        for _,i in pairs(tileBreak) do
                            if getTile(ex - 1,ye + i).fg == 0 and getTile(ex - 1,ye + i).bg == 0 then
                                ...
                                if restBot then
                                    restBotOS(worldBreak,doorBreak,ex,ye)
                                end
                            end
                        end
                    end
...
function plant(world)
    for _,tile in pairs(getTiles()) do
        if (tile.flags ~= 0 and tile.y ~= 0 and getTile(tile.x,tile.y - 1).fg == 0) and findItem(itmSeed) > 0 and getBot().world:upper() == world:upper() then
            if findItem(itmSeed) == 0 and autoFill then
                sleep(100)
                take(world)
                sleep(100)
            end
